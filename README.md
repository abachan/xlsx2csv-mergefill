...
- **ブック名依存式の不整合を補正** — ファイル名変更前の計算値が残った場合、安全に判定できる式だけを現在のブック名で補正します。
...
- `None` は空文字列として出力します。
- 数式セルは、原則としてExcelファイル内に保存されている計算済みの値を取得します。計算済みの値が保存されていない場合は空になることがあります。
- `CELL("filename")` からブック名を抽出する式については、ファイル名変更前の保存済み値が残っていても、式全体がパスやシート名に依存しないと安全に確認できた場合だけ、現在の `.xlsx` ファイル名でメモリ上の値を補正します。
- セル値のないワークシートおよびグラフシートは、0バイトの空CSVになります。

### ブック名依存式の補正範囲

補正対象は `CELL("filename")` と、ブック名抽出に用いられる `MID`、`SEARCH`、`FIND`、`LEN`、`LEFT`、`RIGHT`、`SUBSTITUTE`、文字列連結および基本算術だけで完結する式です。式を異なるパス・シート名のコンテキストで評価し、結果が同一になることを確認してから現在のブック名を採用します。対象外の関数、セル参照、外部参照、パスまたはシート名に依存する式は推測で変更せず、従来どおり保存済みの値を返します。

評価は許可した構文だけを扱う内部パーサーで行い、Pythonの動的評価や外部プログラムは使用しません。入力XLSXは変更せず、補正が必要なOOXML部品だけをメモリ上で逐次処理します。補正不要の場合はZIP全体の再圧縮を省略します。スレッドやプロセスによる並列化は行いません。

実行時依存パッケージは従来どおり `openpyxl` だけです。このライブラリはLibreOfficeやMicrosoft Excelを探索・起動せず、外部の数式再計算オブジェクトを受け取るAPIも持ちません。
